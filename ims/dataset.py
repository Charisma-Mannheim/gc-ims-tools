...
class Dataset:
    ...
    def _measurements(path, subfolders, extension=None, label_keywords=None):
        ...
        if subfolders:
            ...
            # --- Label assignment ---
            default_keywords = ["blank", "sample", "qc", "control"]
            keywords = label_keywords if label_keywords is not None else default_keywords
            labels = []
            for fname in files:
    ...
    def read_mea(cls, path, subfolders=False, sequential=False, compr="wavecompr", extension=".mea", **kwargs):
        ...
        * Data
        ...
        If subfolders=False, labels are generated by searching for keywords in file names. 
        Default keywords are "blank", "sample", "qc", and "control" for example: 
        *   blank_air.csv --> label "blank"
        *   control.csv --> label "control".
        *   All files that do not match any of the keywords are assigned the label "sample".
        A list of custom labels can be passed with the label_keywords argument.

        
        If sequential=True files are compressed with the chosen compr method ("wavecompr" or "binning").
        full dataset afterwards. Especially useful for big datasets or systems with lower RAM specifications.
        ...
        extension : str or list of str, optional
            File extension(s) to include, by default ".mea"

        label_keywords : list of str, optional
            List of keywords to search for in filenames to assign labels.
            If not provided, defaults to ["blank", "sample", "qc", "control"].
    ...
    @classmethod
    def read_zip(cls, path, subfolders=False, extension=[".csv",".json"], **kwargs):
        """
        ...
        *   All files that do not match any of the keywords are assigned the label "sample".
        ...
        extension : str or list of str, optional
            File extension(s) to include, by default [".csv", ".json"]
    ...
    @classmethod
    def read_csv(cls, path, subfolders=False, extension=".csv", **kwargs):
        """
        ...
        *   All files that do not match any of the keywords are assigned the label "sample".
        ...
        extension : str or list of str, optional
            File extension(s) to include, by default ".csv"
